...
```
Almost 60% of the hotels have 4 or higher ratings. We can see that the average distance from the city center is about 0.1 kilometer higher in case of the highly rated hotels, the standard deviations are about the same. The number of hotel stars range between 1 and 5 stars in both rating groups, however the median number of stars is 4 in case of the highly rated, and 3 in case of the hotels with lower ratings. We can also see that the number of rating reviews range in a wide scale including observations with very few ratings. For the subsequent analysis, I excluded hotels with less than 50 reviews to avoid the noise caused by ratings with few reviews.I also kept observations of all type of accommodation in the dataset.
...
In the LPM model the predicted coefficient of stars is 0.229 and statistically significant at 1%. Comparing hotels with the same distance from the city center and with the of the same type, hotels with one additional star are 22.9 percentage point more likely to be highly rated. Distance, has positive coefficient suggesting that, conditional on other characteristics, hotels 1 kilometer farther away from the city center are 2.7 percentage point more likely to have high rating. In case of the accommodation types, the control group is the hotels. We can see that Apartments are 7.7, pensions are 4.4 and hostels are 19.5 percentage points less likely to have 4 or above ratings than hotels conditional on the other characteristics, however these coefficients are statistically not significant. 


The marginal differences from the logit and probit models are almost the same as the corresponding coefficients from the LPM model. If we look at the number of stars, we can see that hotels located at the same distance from the center and of the same type have 21.2 and 21.5 percentage points higher chance to be highly rated if they have one star higher hotel rating. The coefficients and the robust standard errors of the distance from the city center are technically the same as from the LPM model, but similarly, they are statistically not significant. The logit and probit models indicate that Apartments are 9.7 and 9.2 percentage point less likely to have high hotel ratings than hotels, conditional on the other attributes of the hotels. Overall, we can conclude that the three estimated models give very similar results.
...
Let's also compare the goodness of fit measures of the three pobability models.
...
\centering
\begin{tabular}{lccc}
\multicolumn{4}{c}{Table 2: Statistics of goodness of fit for the probability predictions} \\ \midrule
...
As expected, the performance of the models is very similar. The predictions of the logit and the probit models are slightly better in case of the Brier score and little bit worse in case of the Log-loss compared to the LPM model, but the differences are not considerable. Hence, we can conclude that using the LPM model to uncover the associations is just as fine as the logit or the probit models, but if we care about prediction we can choose to use the latter two models instead.


### Predictions

The following figure plots the predictions from the logit and probit models on the y axis against the predictions from the LPM model on the x axis. We can clearly see that the logit and probit predictions move closely together. If we compare them to the LPM model, we can see that the scatterplot of the predicted values slighly vary from the LPM predictions which is more visible in the tails.
...
Finally, let's take a quick look at the predicted probabilities of being highly rated by the actual rating categories in the dataset.
