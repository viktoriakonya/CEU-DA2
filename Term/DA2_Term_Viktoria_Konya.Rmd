...
In order to test the hypothesis about the pattern between the ICT resources and test scores, I will assume that two groups of predictors impact the students' academic performance, hence needs to be controlled for: the background characteristics of the students and the characteristics of the students' school. The specification of the theoretical model is the following:
...
Where the outcome $Test\ \ score$ is the test score of mathematics, reading or science, $No\ \ computer$ is a dummy showing if the student's household do not own any computers, $Student_{k,i}$ is a vector with the student's characteristics and family background variables and finally, $School_{l,i}$ contains the the attributes of the students' school. In this analysis I will only consider the students' background variables as controls and ignore the school effect despite we know that schools have considerable contribution to students' academic success. From the student's predictors the following variables will be considered: the students' index of socio-economic status "SES" (ESCS), the household's access to internet, the student's gender, education of parents, possession of an own room or own desk and the approximate number of books in the household. If there is academic loss due to the lack of access to ICT resources, then we expect to have significant negative estimate of the $No\ \ computer$ even after controlling for the students individual and family background variables. I will present the detailed results in the main text using the maths score as outcome variables. Results for the read score can be found in the *Appendix*.
...
First, let's take a look at the variable of our interest. The first panel shows the average test scores among student who have and do not have computers in their households. As expected, students without access to computers in their homes have lower performance, on average. The average test score disadvantage of students without computers in their homes is 64 test scores. The figures imply considerable academic loss due to the lack of ICT resources in the household, however we cannot yet draw conclusions as there are several other factors that determine the academic performance of the students. We expect this gap to decrease as we include more of the observable attributes of the students but some of the loss in test scores should remain in order to find evidence for academic inequalities by the access to ICT resources.

It is also worth to take a look at the pattern of association between the outcome and the controlling variables. The second panel shows the scatter plot of our key control variable, the SES index against the test scores together with the lowess smoother. It seems that the relationship is positive and linear as we assumed, but after about the SES index of 2 it sharply decreases. It would imply that higher socio-economic status contributes to the academic performance until a certain level, but the performance of the students with the highest socio-economic status declines. In order to capture the potential non-linearity in the relationship, we can try to fit a linear piecewise spline and test if we can find evidence for a negative association between the test scores and the second segment of the SES index at all. 

For the predictors **Appendix A.3.** contains the corresponding visualizations of the joint distributions and it also contains the same for the read scores. As the rest of our predictors are either binary or ordinal, in this case we can take a look at the average test scores by the the values of the qualitative variables to have an idea which can be the potentially strong predictors of the test results. The average test scores do not seem to differ much by the gender of the students, by the own room and an own desk flags. However, test scores monotonically increase with the parents' education and with the number of the books owned in the household. It implies that the latter two predictors should be also in the focus of the regression analysis.
...
The purpose of the regression analysis to uncover and quantify the extent of the academic loss due to the access to the ICT resources while we also consider other characteristics of the students. The detailed model building steps can be found in the *Appendix A.4.* while in this section only the selected model specifications are presented.
...
The results imply that the academic disadvantage of students without ICT resources at home is 65 test scores without controlling for the students' observable characteristics. However, the explanatory power of the first model is quite poor with only explaining about 4 percent of the overall variation of the math scores. When we include the SES index of the students in our second model the ICT gap decreases to 23 scores as well as the the $R^{2}$ increases to 22 percent. It suggests that, as we expected, that the socio-economic background is the strongest predictor of the academic performance. In the third regression, more background variables of the students were added, including the education of the parents, the students' gender, the approximate number of books in the household and weather the students own a desk or a room and the household has Internet connection. The level of education of the parents and the number of books owned are both positively associated with the test performance, only the 500 or above books owned seems to break the monotonicity of the relationship. Female students have, on average, lower performance in the math test and owning a desk does not seems to influence the test scores. Moreover, the Internet connection is statistically not significant in the model which can be due to the low number of observations and its strong association with the computer field. Overall, after including all the available important characteristics of the students the academic loss due to the lack of ICT resources is 17 scores in the maths tests. In regression four, linear piecewise spline was included to capture the negative pattern at the high level of the SES index. It seems that the more complicated pattern does not change much on the explanatory power of the model, hence we can use the third model to make robustness checks.
...
Now that we examined the relationship between the access to ICT resources and the maths scores in Hungary, let's make some robustness checks weather we can find same patterns in case of the test scores in other subjects, in other countries and in other years when the test took place.

Firstly, let's take a look at at the ICT gap using the reading and science test scores as outcome variables. Appendix *A.5.* contains the results of regressions with only the ownership of the computer and using the full model with all student background variables. The figures imply very similar academic disadvantages to what we saw using the math scores as dependent variable which is about 21 and 19 scores for those without computers at their homes.

Another interesting question is weather we can see the same pattern in other countries. In order to test the similarity in the pattern across countries, the same model was applied on the PISA survey results of the three other V4 countries and on six Western-European countries. *Appendix A.6.* contains the detailed results of the regressions, i the main text only the estimated parameter of owning a computer is presented.
...
ggplot(SI, aes(x = coeff, y = factor(model, level = c("HUN", "POL", "CZE", "SVK", "AUT", "ESP", "GBR", "ITA", "FRA", "DEU")), xmin = conf_low, xmax = conf_high)) +
  ...
  geom_point(size = 2) +
  theme_bw() +
  labs(x = "Difference in math test scores", y= "", color = "", caption="Note: Markers show the estimated parameter of owning a computer and error bars \n show the 95% CI. The Unconditional estimate shows the gap without control \n variables and the conditional estimate shows the same for the full modell  with \n the control variables discussed in Table2. ") +
...
The unconditional ICT gap seems to be the strongest in Slovakia and Germany, where students without ICT resources suffer more than 80 loss in test scores, on average. When all student background attributes are controlled for, some part of this disadvantage diminishes in all the examined countries, just like we have seen from the regression analysis of Hungary. If we compare the conditional gap with other countries, Hungary seems to have the smallest negative gap together with Slovakia, Poland and Britain. It can either mean that the disadvantage in academic loss due to the lack of ICT devices is moderate in Hungary compared to other European countries or that the students' performance is strngly related to the other background variables, such as the SES index or the education of parents. If we look at *Table 5* in *Appendix A6* it seems that the parents' education adds most to the performance of the student in Hungary among the examined countries.

We can make another stability check by comparing different PISA survey results. The evolution of the ICT gap over the past three PISA test in Hungary is shown in *Appendix A.7.*. It seems that the academic loss due to the ICT gap , both conditional and unconditional,  slightly decreased over the 3 years from 23 to 18 scores but the gap seems fairly stable over the years.
