...
title: "Education inequalities and lack of access to ICT resources"
...
# Unconditional
reg1 <- feols(math ~ has_computer, data=df, vcov = 'hetero' , weights = df$stu_wgt)
...
Intercept                                                      & 489.65***            & 494.70***            & 394.17***                   & 396.16***                   \\
                                                               & (1.35)               & (1.13)               & (5.80)                      & (5.79)                      \\
Computer                                                       & -68.31***            & -23.69***            & -19.30***                   & -18.95***                   \\
                                                               & (4.77)               & (4.47)               & (4.54)                      & (4.54)                      \\
SES                                                            &                      & 43.34***             & 15.11***                    &                             \\
                                                               &                      & (1.27)               & (1.83)                      &                             \\
Internet                                                       &                      &                      & 4.99                        & 5.35                        \\
...
```{r main-regression-countries, echo = F, include = F, warning = F, meassage = F}
...
# Visualize results
p1 <- ggplot(SI, aes(x = coeff, y = factor(model, level = c("HUN", "POL", "CZE", "SVK", "AUT", "ESP", "GBR", "ITA", "FRA", "DEU")), xmin = conf_low, xmax = conf_high)) +
  geom_errorbarh(height=.2, aes(color = type), size = 1) +
  geom_point(size = 2) +
  theme_bw() +
  labs(x = "Difference in math test scores", y= "", color = "") +
  scale_color_manual(values = c("#00AFBB", "#2ca25f")) +
  scale_x_continuous(limits = c(-110, 0), breaks = seq(-120, 0, by = 20)) +
  ggtitle("Fig2.a: ICT gap in European countries") + 
  theme(
...
```{r main-regressions-years, echo = F, include = F, warning=F, message=F}

# Regresssions for each year

# Summary results of HUN 2018, 2015 and 2013 comparison
# HUN - 2018
df2 <- df0 %>% filter(country == 'HUN' & year == 2018)
reg_cu_hu_1 <- feols(math ~ has_computer , data = df2, vcov = 'hetero', weights = df2$stu_wgt )
reg_c_hu_1 <- feols(math ~ has_computer + escs + as.factor(parents_education) + female + has_room + as.factor(books) + has_room + has_desk, data = df2, vcov = 'hetero', weights = df2$stu_wgt )

# HUN - 2015
df2 <- df0 %>% filter(country == 'HUN' & year == 2015)
reg_cu_hu_2 <- feols(math ~ has_computer , data = df2, vcov = 'hetero', weights = df2$stu_wgt )
reg_c_hu_2 <- feols(math ~ has_computer + escs + as.factor(parents_education) + female + has_room + as.factor(books) + has_room + has_desk, data = df2, vcov = 'hetero', weights = df2$stu_wgt )

# HUN - 2012
df2 <- df0 %>% filter(country == 'HUN' & year == 2012)
reg_cu_hu_3 <- feols(math ~ has_computer , data = df2, vcov = 'hetero', weights = df2$stu_wgt )
reg_c_hu_3 <- feols(math ~ has_computer + escs + as.factor(parents_education) + female + has_room + as.factor(books) + has_room + has_desk, data = df2, vcov = 'hetero', weights = df2$stu_wgt )


msummary(list("2012 - HUN" = reg_cu_hu_3, "2012 - HUN" = reg_c_hu_3, "2015 - HUN" = reg_cu_hu_2, "2015 - HUN" = reg_c_hu_2, "2018 - HUN" = reg_cu_hu_1,  "2018 - HUN" = reg_c_hu_1 ),
         fmt="%.2f",
         gof_omit = 'DF|Deviance|Log.Lik.|F|AIC|BIC|R2 Within|R2 Pseudo|Std.Errors',
         stars=TRUE,
         estimate = "{estimate}{stars} ", 
         ...
         coef_rename = varname_report,
         title = "Dependent variable: Math score",
         notes = "Note: Data of PISA survey 2018, 2015 and 2012, Hungary.") %>% 
        kable_classic(full_width = F, position = "center" , font_size = 9) 
...
for (i in 1:3) {
  ...
  SI$model[i] <- "HUN"
  SI$year[i] <- years[i]
  SI$coeff[i] <- eval(as.name(paste0('reg_c_hu_',i)))$coefficients[2]  
  SI$se[i] <- eval(as.name(paste0('reg_c_hu_',i)))$se[2]  
  SI$conf_low[i] <- SI$coeff[i] - 1.96 * SI$se[i]  
  SI$conf_high[i] <- SI$coeff[i] + 1.96 * SI$se[i]  
...
for (i in 4:6) {
  ...
  SI$model[i] <- "HUN"
  SI$year[i] <- years[i]
  SI$coeff[i] <- eval(as.name(paste0('reg_cu_hu_',i-3)))$coefficients[2]  
  SI$se[i] <- eval(as.name(paste0('reg_c_hu_',i-3)))$se[2]  
  SI$conf_low[i] <- SI$coeff[i] - 1.96 * SI$se[i]  
  SI$conf_high[i] <- SI$coeff[i] + 1.96 * SI$se[i]  
...
p2 <- ggplot(SI, aes(x = coeff, y = factor(year, level = c("2013", "2015", "2018")), xmin = conf_low, xmax = conf_high)) +
  geom_errorbarh(height=.2, aes(color = type), size = 1) +
  geom_point(size = 2) +
  theme_bw() +
  labs(x = "Difference in math scores", y= "", color = "") +
  scale_color_manual(values = c("#00AFBB", "#2ca25f")) +
  scale_x_continuous(limits = c(-110, 0), breaks = seq(-120, 0, by = 20)) +
  ggtitle("Fig2.b: ICT gap in Hungary") +
  theme(
...
\newpage
### A2. Histograms of the test scores and ESCS
...
# Visualize
grid.arrange(grobs = list(hist_math, 
             hist_read, 
             hist_science,
             hist_escs),
             ncol=2,
...
\newpage
### A3. Relatinship with outcome variables (math and read scores)
...
bin_scatter_math_pareduc <- bin_scatter_math_pareduc + scale_x_discrete(labels = c("0-8 years", "10.5 years", "12 years",  "NA")) 
...
bin_scatter_read_pareduc <- bin_scatter_read_pareduc + scale_x_discrete(labels = c("0-8 years", "10.5 years", "12 years",  "NA")) 
...
bin_scatter_math_books <- bin_scatter_math_books + scale_x_discrete(labels = c("0-10", "11-25", "26-100",  "101-200", "201-500", "500+", "NA")) 
...
bin_scatter_read_books <- bin_scatter_read_books + scale_x_discrete(labels = c("0-10", "11-25", "26-100",  "101-200", "201-500", "500+", "NA")) 
...
\newpage
```{r A3-relatinship-with-outcone2, echo = F, fig.align='center', fig.height=10, fig.width=10, warning=F, message=F}
...
msummary(list("(1)" = reg1, "(2)" = reg2, "(3)" = reg3, "(4)" =  reg4, "(5)" = reg5, "(6)" = reg6, "(7)" = reg7, "(8)" = reg8, "(9)" = reg9),
         ...
         statistic = "std.error",
         coef_rename = varname_report,
         title = "A.4. Regression results - Dependent variable: Math score",
         notes = "Note: Data of PISA survey 2018, Hungary.") %>% 
        kable_classic(full_width = F, position = "center" , font_size = 9) %>% 
...
msummary(list("Read" = reg1_r, "Read" = reg8_r, "Science" = reg1_sc, "Science"  = reg8_sc),
         ...
         #coef_group = groupConf,
         coef_rename = varname_report,
         title = "A.5. Robustness check by subjects - Dependent variable: Reading / Science score",
         notes = "Note: Data of PISA survey 2018, Hungary.") %>% 
        kable_classic(full_width = F, position = "center" , font_size = 9)
...
msummary(list("HUN" = reg_c_1,  "POL" = reg_c_2, "CZE" = reg_c_3, "SVK" = reg_c_4, "AUT" = reg_c_5,"ESP" = reg_c_6,"GBR" = reg_c_7,"ITA" = reg_c_8, "FRA" = reg_c_9, "DEU" = reg_c_10),
         ...
         #coef_group = groupConf,
         coef_rename = varname_report,
         title = "A.6. Robustness check by countries - Dependent variable: Math score",
         notes = "Note: Data of PISA survey 2018.") %>% 
        kable_classic(full_width = F, position = "center" , font_size = 9) %>% 
  kableExtra::landscape()
...
reg_c_1 <- feols(math ~ has_computer + has_internet + escs + as.factor(parents_education) + female + has_room + as.factor(books) + has_room + has_desk, data = df2, vcov = 'hetero', weights = df2$stu_wgt )

# HUN - 2015
df2 <- df0 %>% filter(country == 'HUN' & year == 2015)
reg_cu_2 <- feols(math ~ has_computer , data = df2, vcov = 'hetero', weights = df2$stu_wgt )
reg_c_2 <- feols(math ~ has_computer + has_internet + escs + as.factor(parents_education) + female + has_room + as.factor(books) + has_room + has_desk, data = df2, vcov = 'hetero', weights = df2$stu_wgt )

# HUN - 2012
df2 <- df0 %>% filter(country == 'HUN' & year == 2012)
reg_cu_3 <- feols(math ~ has_computer , data = df2, vcov = 'hetero', weights = df2$stu_wgt )
reg_c_3 <- feols(math ~ has_computer + has_internet + escs + as.factor(parents_education) + female + has_room + as.factor(books) + has_room + has_desk, data = df2, vcov = 'hetero', weights = df2$stu_wgt )
...
msummary(list("2012 - HUN" = reg_cu_3, "2012 - HUN" = reg_c_3, "2015 - HUN" = reg_cu_2, "2015 - HUN" = reg_c_2, "2018 - HUN" = reg_cu_1,  "2018 - HUN" = reg_c_1 ),
         ...
         #coef_group = groupConf,
         coef_rename = varname_report,
         title = "A.7. Robustness check by years - Dependent variable: Math score",
         notes = "Note: Data of PISA survey 2018, 2015 and 2012, Hungary.") %>% 
        kable_classic(full_width = F, position = "center" , font_size = 9) 
